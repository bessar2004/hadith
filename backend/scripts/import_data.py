...
def import_data(limit: int | None = None):
    ...
    for ar_row in tqdm(ar_rows, desc="Import", unit="hadis"):
        ...
        kitap     = ar_row.get("kitap", "Sahih al-Bukhari").strip()
        bab       = ar_row.get("bab", "").strip()
        ravi      = ar_row.get("ravi", "").strip()
        kaynak    = ar_row.get("link", "").strip()

        # Arapça
        ar_sanad  = ar_row.get("sanad", "").strip()
        ar_detail = ar_row.get("hadith_text", "").strip()
        ar_full   = f"{ar_sanad} {ar_detail}".strip()
        ar_temiz  = clean_arabic(ar_full)

        # İngilizce
        en_row    = en_map.get(row_id, {})
        en_sanad  = en_row.get("sanad", "").strip()
